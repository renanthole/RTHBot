@extends('layouts.app', [
    ...
    'breadcrumbs' => [['url' => route('quizzes.index'), 'label' => __('Questionários')], ['url' => route('questions.index', $quiz->id), 'label' => __('Perguntas')], ['url' => route('questions.create', $quiz->id), 'label' => __('Nova pergunta')]],
])
    ...
    <form action="{{ route('questions.store', $quiz->id) }}" method="POST">
        ...
        <div class="card card-outline card-primary">
            <div class="card-header">
                <h3 class="card-title">{{ __('Nova pergunta') }}</h3>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-12">
                    ...
                    <div class="col-md-12">
                        <div class="form-group {{ $errors->has('question') ? 'has-danger' : '' }}">
                            ...
                            <textarea name="question" id="question" class="form-control {{ $errors->has('question') ? 'is-invalid' : '' }}"
                                cols="30" rows="5" required>{{ old('question') }}</textarea>

@extends('layouts.app', [
    ...
    'breadcrumbs' => [['url' => route('quizzes.index'), 'label' => __('Questionários')], ['url' => route('answers.index', $quiz->id), 'label' => __('Respostas')], ['url' => route('answers.create', $quiz->id), 'label' => __('Nova resposta')]],
])

@section('content')
    <form action="{{ route('answers.store', $quiz->id) }}" method="POST">
        @csrf
        @method('POST')
        <div class="card card-outline card-primary">
            <div class="card-header">
                <h3 class="card-title">{{ __('Nova resposta') }}</h3>
            </div>
            <div class="card-body">
                <div class="row">
                    <div class="col-md-12">
